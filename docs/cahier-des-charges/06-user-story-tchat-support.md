...
- Étant donné une coupure réseau, quand la connexion est perdue, alors le client en est informé et la session se reconnecte (dégradation gracieuse et reconnexion) ; l'historique de la conversation est préservé.
...
- **Preuve de concept** : périmètre strict, Customer et Agent, connexion authentifiée, échange, isolation ; harness HTML nu pour la démonstration ; aucune stack d'identité réimplémentée.
