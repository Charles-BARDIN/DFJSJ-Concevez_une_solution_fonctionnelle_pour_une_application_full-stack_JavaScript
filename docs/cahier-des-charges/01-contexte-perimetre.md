...
| **API (CRUD par domaine)** | Exposée aux **applications d'agence tierces existantes** (utilisateur, réservation, offre, agence…), conformément à l'exigence du v0. Ces applications sont des **composants tiers à intégrer** (modélisés au Stade 3, C.1.7), **non développés ici**. |
