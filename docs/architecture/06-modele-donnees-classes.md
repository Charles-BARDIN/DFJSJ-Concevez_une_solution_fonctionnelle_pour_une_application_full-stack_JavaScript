...
- **Offer** (offre : id, agence de retrait, agence de retour, catégorie ACRISS, début / fin
  horodatés — date et heure (`start_at` / `end_at`, v0), price_amount et price_currency) — ses
...
- **Payment** *(transactionnel)* : 0..n par réservation (un paiement initial, plus d'éventuels
  ...
  transaction_reference opaque, status, amount et currency, aucune donnée de carte
  (§6.4).
...
- `Payment` stocke une direction (`charge` | `refund`), une référence de transaction opaque
  (identifiant rendu par le prestataire), un statut et un montant et devise, ce qui rend
...
- **Agence de retrait ≠ agence de retour** (ADR-012) : deux clés étrangères distinctes sur `offer`
  ...
  `agency.city_id`.
- **Paiement agnostique** (`NFR-SEC-05`) : `payment` ne porte qu'une direction et référence
  opaque, statut et montant / devise (§6.4).
...
> il supporte N participants par conversation. La règle « Customer et Agent, deux côtés » de la
