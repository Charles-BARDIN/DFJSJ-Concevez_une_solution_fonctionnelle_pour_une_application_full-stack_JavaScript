...
gestion.
**Alternatives écartées.** Concevoir « deux surfaces » (application client et application back-office)
...
**Décision.** Un harness HTML nu à deux entrées (Customer et Agent) : un champ de saisie +
...
personnelles » et paiement. Pas de mention du permis ni de l'âge.
...
**Conséquences.** US-CHAT-01 / US-CHAT-02 (support tchat). Périmètre PoC strict (Customer et Agent, handshake
...
Justification : tokens courts et secret révocable / rotable corrigent l'esprit d'`AUD-12` au lieu de
...
divergents, codebases par pays et échanges manuels : une divergence subie. Ici : un
...
la fragmentation.
**Alternatives écartées.** Région unique et clauses contractuelles types / décision d'adéquation →
...
(répartiteur, N instances et base régionale). Le schéma et le contrat d'API restent uniques.
...
(PCI-DSS délégué, `NFR-SEC-05`). Couverture UE et Amérique du Nord avec SCA / 3-D Secure,
