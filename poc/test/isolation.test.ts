...
test('a non-participant AGENT is refused too — isolation is by membership, not role', async (t) => {
  const db = openDatabase(':memory:');
  applySchema(db);
  const repo = new SqliteChatRepository(db);

  // C1 has two legitimate participants; 'u-agent-2' is an authenticated support
  // agent but a participant of no conversation — in particular, not of C1. Isolation
  // rides on the Participant -> Conversation relation (findParticipant), never on the
  // seat role: an agent seat earns no access it was not granted as a participant.
  const c1 = repo.createConversation({ status: 'open', createdAt: '2026-06-29T10:00:00.000Z' });
  const customer = repo.addParticipant(c1, 'u-customer', 'customer');
  const agent = repo.addParticipant(c1, 'u-agent', 'agent');
  if (!customer.ok) throw new Error('failed to seed the customer');
  ...
  const customerWs = connect(url, 'u-customer', 'client');
  const agentWs = connect(url, 'u-agent', 'support_agent');
  const intruderAgentWs = connect(url, 'u-agent-2', 'support_agent');
  await Promise.all([once(customerWs, 'open'), once(agentWs, 'open'), once(intruderAgentWs, 'open')]);

  t.after(async () => {
    customerWs.terminate();
    agentWs.terminate();
    intruderAgentWs.terminate();
    await new Promise<void>((resolve, reject) => server.close((err) => (err ? reject(err) : resolve())));
    db.close();
  });
  ...
  // (a) The intruding agent targets C1 with an otherwise valid frame and must be refused.
  const refused = once(intruderAgentWs, 'message', { signal: AbortSignal.timeout(2000) });
  intruderAgentWs.send(JSON.stringify({ type: 'send', conversationId: c1, body: 'let me into a conversation that is not mine' }));
  const [rawRefusal] = await refused;
  const refusal = JSON.parse(rawRefusal.toString());
  assert.equal(refusal.type, 'refusal');
  assert.equal(refusal.reason, 'isolation_denied');
  ...
  // (b) Nothing persisted.
  assert.equal(repo.listMessages(c1).length, 0);
  // (c) Nothing delivered to legitimate members — including the conversation's own agent.
  assert.equal(customerReceived, false);
  assert.equal(agentReceived, false);

  // Non-regression: the refusal did not break the channel — the legitimate agent (a
  // member of C1) still reaches the customer.
  const delivered = once(customerWs, 'message', { signal: AbortSignal.timeout(2000) });
  agentWs.send(JSON.stringify({ type: 'send', conversationId: c1, body: 'reply from the member agent' }));
  const [rawDelivered] = await delivered;
  assert.equal(JSON.parse(rawDelivered.toString()).body, 'reply from the member agent');
  assert.equal(repo.listMessages(c1).length, 1);
});
